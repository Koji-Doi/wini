...
# cit() -- {{ref|...}}1 マクロの内容の解析・データ取り込み・仮ラベルの出力

## synopsis

* 読み取った文献情報をグローバル変数である%REFに蓄える。
* 出力される「仮ラベル」は最終的にderef()によって適切な文献番号を付された正式なHTMLテキストに置き換えられる。
...
# ylml() -- 変数定義パートの解析とデータ取り込み（yaml-like markup language)
